...
const joinUserSchedulesByUser = (userScheduleArr) => {
  ...
  let currData = userScheduleArr[0];
  // console.log('userScheduleArr', userScheduleArr)

  userScheduleArr.forEach(u => {
    const isSameUserData = currData.user_id === u.user_id;
    // console.log('u', u)
    // console.log(isSameUserData)

    if (!isSameUserData) {
      // console.log('currData', currData)
      pushDataToSchedule(currData);
      currData = u;
      weekDates = {};
    }
    ...
    if (u.has_schedule) {
      ...
      let day = u.date_start.getDay() - 2;
      if (day < 0)
        day += 7;

      weekDates[day] = {
        turnId: u.turn_id,
        dateStart: u.date_start,
        dateEnd: u.date_end,
        dateLunch: u.date_lunch
      }
    }
  })
