...
const calculateTotalHours = weekDates => {
  ...
  for (let dates of weekDates) {
    ...
    if (dates) {
      totalHours += Math.abs(new Date(dates.dateEnd) - new Date(dates.dateStart)) / 36e5;
      // const dummyDate = new Date(dates.dateStart)
      // const newDate = new Date(dates.dateEnd) - new Date(dates.dateStart)
      // console.log('\n\nnewDate / 36e5', newDate / 36e5)

      // const n = new Date(dates.dateEnd) - new Date(dates.dateStart)
      // // console.log('', dates.dateStart.getUTCMinutes(), dates.dateEnd.getUTCMinutes(), dates.dateLunch.getUTCMinutes())

      // // const assignedHours = dates.dateStart.getUTCMinutes() - dates.dateEnd.getUTCMinutes() - dates.dateLunch.getUTCMinutes();
      // const nStart = new Date(dates.dateStart);
      // const nEnd = new Date(dates.dateEnd);
      // const nLunch = new Date(dates.dateLunch);
      // console.log('ns = ', Math.abs(nEnd - nStart))
      // console.log(nStart.getMilliseconds(), nStart.getUTCMilliseconds(), Math.floor(nStart.getMilliseconds() / 60000))

      // const assignedHours = Math.abs(nEnd - nStart) / 36e5;
      // console.log('assignedHours', assignedHours)

      // const isoYearFormat = toISOYearFormat(newDate);
      // // console.log('isoYearFormat', isoYearFormat)
      // // console.log('-', dummyDate, dummyDate.getUTCHours(), dummyDate.getUTCMinutes(), dummyDate.getSeconds())

      // // const newTime = get24HourFormatOfTime(turn.timeEnd, true);
      // // console.log('newTime', newTime)

      // // const dateWith = new Date(`${isoYearFormat}T${newTime}Z`).toISOString();
      // // console.log('dateWith', dateWith)

      // totalHours += Math.abs(n) / 36e5;
      // // console.log('acc', totalHours)
    }
  }
  return totalHours;
}
